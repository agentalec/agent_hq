...
6. **Budgets** (`budgets.yml`): `ticket_cap_usd`, `in_flight_cap`, `loop_guard.max_runs` (the only structural ceiling -- there is no `max_depth`) — with the caveat that under the default `copilot-cli` binding, USD caps (`ticket_cap_usd`, per-task `budget.max_cost_usd`) do not bind because runs record `cost_usd: 0.0` (`docs/architecture.md`, deviation 9); retries, loop guard, in-flight cap, and runtime deadlines still do.
