...
2. **Engine repo and intake** (`projects.yml`): `engine_repo` (the engine's own issue tracker — distinct from work repos), `intake_label`, `initial_task`, `final_task` (the run whose completion finishes a ticket — a queue draining anywhere else blocks it), `comment_default_task` (what a bare approver comment on the engine issue queues; omit to require an explicit `/agent-hq` command), `intake.min_body_words`, `intake.excluded_labels`, and `public` / `public_safe_label` (when `public: true`, every ticket needs the safe label).
...
6. **Budgets** (`budgets.yml`): `ticket_cap_usd`, `in_flight_cap`, `loop_guard.max_runs` (the only structural ceiling -- there is no `max_depth`), `max_queue_length` (entries one run may declare), `max_comment_runs_per_ticket` (comment-triggered runs per ticket, separate from `max_runs`) — with the caveat that under the default `copilot-cli` binding, USD caps (`ticket_cap_usd`, per-task `budget.max_cost_usd`) do not bind because runs record `cost_usd: 0.0` (`docs/architecture.md`, deviation 9); retries, loop guard, in-flight cap, and runtime deadlines still do.
...
10. Always run `.venv/bin/agent-hq config validate`. Also run `.venv/bin/agent-hq tasks validate` when the change touches `initial_task`/`final_task`/`comment_default_task`/`feedback_task` (all must name a loaded task), gate bindings/named gates, approver group names, or anything a `task.yml` references.
