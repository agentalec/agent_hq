...
| Breadth-aware run cap | `loop_guard.max_runs` counts every run on a ticket regardless of which repo it targets, so it cannot tell "five repos progressing" from "one repo looping". Measured on the pilot's tickets the since-deleted `max_depth` was the guard that actually bound (depth == non-failed runs - 1: t11 7/6, t17 7/6, t30 12/8), which is why #46 removed it as redundant — leaving `max_runs`, still tuned for one linear single-repo route. Three repos × implement/review with one review round each, plus spec/qa/finalize, is already ~15-20 runs; five exceeds 25 with no lane having looped, and the cap blocks a converging ticket. Fix is a per-lane (per-`repo`) cap or one scaled by the declared queue's breadth — not a bigger single number | The first ticket whose `spec` fans out to more than one repo. Fires with **Planned #3** (`parallel_ok` fan-out) and must land *with* it, not after |
...
- Task library: `clinical`, `poll`, `docs` (with their gates, PHI lint, docs-drift blocking). Task definitions + minimal skills exist under `tasks/` (validated, but no prompt queues them); still pending: the `github-issue-reactions` adapter for `poll`, and a prompt that names each of these in a queue declaration. There is no activation edit — the explicit-queue change deleted `handoff.allowed`, so every library task is already queueable. `qa` is now **wired** (`review` → `qa` → `finalize`) — it runs Playwright from the repo's own tooling inside the devcontainer and screenshots each acceptance criterion onto the PR, so it did not wait on the `docker-compose` qa-env adapter; that adapter stays deferred in section A for the stacks the devcontainer can't stand up.
