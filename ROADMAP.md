...
| #16 | Gate comments are reviewable: the approval request inlines every artifact the gated run produced (collapsed `<details>`, truncated past 20000 chars), and a `WAITING_GATE` run labels its issue `hq:waiting-gate` until a decision lands | `engine/adapters/github_issue_comment_gate.py`, `engine/engine.py`, `engine/runner.py` |
